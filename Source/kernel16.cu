...
//-------------------------------------------------------------------
__device__ __forceinline__ void add(float4& output, const ushort3& color, const float factor)
{
...
//-------------------------------------------------------------------
__device__ __forceinline__ float distance(const float4& avg, const ushort3& color, const uint32_t pixel_max)
{
	const float x = avg.x - color.x;
	const float y = avg.y - color.y;
	const float z = avg.z - color.z;

	return sqrtf((x * x + y * y + z * z) / (pixel_max * pixel_max * 3)); // L2-Norm / sqrt(255^2 * 3)
}

//-------------------------------------------------------------------
__global__ void kernelGuidance(const ushort3* __restrict__ input, ushort3* __restrict__ patches, const Params p)
{
	...
	for (uint32_t i = WTHREAD; i < l.pixelCount; i += WSIZE)
	{
		...
		const ushort3& pixel = input[x + y * p.iWidth];
	...
	{
		normalize(color);
		patches[PX + PY * p.oWidth] = make_ushort3(color.x, color.y, color.z);
	}
}

//-------------------------------------------------------------------
__device__ __forceinline__ float4 calcAverage(const Params& p, const ushort3* __restrict__ patches)
{
...
//-------------------------------------------------------------------
__global__ void kernelDownsampling(const ushort3* __restrict__ input, const ushort3* __restrict__ patches, const Params p, ushort3* __restrict__ output)
{
	if (PX >= p.oWidth || PY >= p.oHeight) return;
	...
	for (uint32_t i = WTHREAD; i < l.pixelCount; i += WSIZE)
	{
		...
		const ushort3& pixel = input[x + y * p.iWidth];
		float f = distance(avg, pixel, p.pixel_max);
	...
	if (WTHREAD == 0)
	{
		ushort3& ref = output[PX + PY * p.oWidth];

		if (color.w == 0.0f)
			ref = make_ushort3((unsigned short)avg.x, (unsigned short)avg.y, (unsigned short)avg.z);
		else
		{
			normalize(color);
			ref = make_ushort3((unsigned short)color.x, (unsigned short)color.y, (unsigned short)color.z);
		}
	}
}
...
// HOST
//-------------------------------------------------------------------
static void check(cudaError err)
{
	if (err != cudaSuccess)
	{
		FILE * pfile = fopen("dpid_error.txt", "w");
		fprintf(pfile, "CUDA_ERROR: %d %s: %s\n", (int)err, cudaGetErrorName(err), cudaGetErrorString(err));
		fclose(pfile);
...
{
	// calc sizes
	const size_t sInput = sizeof(ushort3) * i.iWidth * i.iHeight;
	const size_t sOutput = sizeof(ushort3) * i.oWidth * i.oHeight;
	const size_t sGuidance = sizeof(ushort3) * i.oWidth * i.oHeight;

	// alloc GPU
	ushort3* dInput = 0, *dOutput = 0, *dGuidance = 0;
